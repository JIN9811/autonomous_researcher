...
아래 숫자는 2026-09-18 작업자가 지정한 시험 조건이다. 원본 시연 영상의 설정이나 장비 공통 상수가 아니며, 이 문서를 수정해도 장비 method가 변경되지는 않는다.

| 설정 | 현재 적용값 | 설정 의도 | 소유 주체 |
|---|---:|---|---|
| 로봇 진입 높이 | 150 mm | 로봇이 시편을 안전하게 반입·회수할 수 있는 지그 간격 확보 | 셀 운용 설정 |
| 접촉 판정 하중 | 10 N | platen과 시편의 실제 접촉점을 검출하고 상대 Stroke 기준점을 설정 | 시험 method |
| 시편 기준 높이 | 30 mm | 현재 시편의 치수와 변형량 계산 기준 제공 | 시편 recipe / 시험 method |
| 시험 종료 Stroke | 현재 method에서 지정 | 특정 종료값을 고정하지 않으며 분석에 필요한 변위 범위 이상을 확보 | 시험 method |
| 최소 분석 가능 변위 | 현재 시편의 초기 높이 × 50% 이상 | 보정된 측정 변위가 현재 시편 높이의 50%에 도달해야 하며, 그 전에 끝난 데이터는 SEA/BO에 사용하지 않음 | Analysis Agent |
| SEA 분석 구간 | 현재 시편의 초기 높이의 0–50% | 해당 구간의 보정된 FD 에너지만 적분해 시편 질량으로 나눔. 시편 높이가 바뀌면 적분 경계도 함께 변경 | Analysis Agent |
| 시험 준비 간격 | 32 mm | 시험 시작 전에 platen을 시편 근처의 준비 위치로 이동 | 시험 method |
| 시험 속도 | 2 mm/min | 현재 준정적 압축시험의 하강 속도 | 시험 method |
