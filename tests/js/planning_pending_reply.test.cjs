...
function setup() {
  ...
  const c=vm.createContext({URLSearchParams,Set,Number,String,Boolean,
    ...
    document:{body:{classList:new Set()}},liveRuntimeTitle:{classList:new Set()},
    liveOrchestratorReady:false,planningChatStatus:{},compactText:s=>s,liveHumanizeStatus:s=>s,
  ...
  vm.runInContext(part('setLiveOrchestratorReady(', 'function relativeTimeLabel(')
    +part('createPlanningPendingMessage(', 'function mergePlanningMessages(')
...
test('restored busy run keeps bootstrap ready glow but clears only its response placeholder',async()=>{
  const {c,network,calls}=setup();
  c.liveLastSession={is_running:true,message_total:395};
  c.liveBackendPlanningBusy=true;
  const p=c.bootstrapLiveOrchestrator();
  assert.equal(calls(),1);
  assert.equal(c.planningMessagesCache.some(m=>m.pendingReasoning),true);
  assert.equal(c.liveOrchestratorReady,false);
  network.resolve({ok:true,json:async()=>({ok:true,session:{
    is_running:true,is_planning_busy:true,messages:[{content:'Original greeting'}]}})});
  await p;
  assert.equal(c.liveOrchestratorReady,true);
  assert.equal(c.document.body.classList.has('live-orchestrator-ready'),true);
  assert.equal(c.liveRuntimeTitle.classList.has('orchestrator-ready'),true);
  assert.equal(c.planningMessagesCache.some(m=>m.pendingReasoning),false);
  assert.equal(c.liveBackendPlanningBusy,true);
  assert.equal(c.planningMessagesCache[0].content,'Original greeting');
});
