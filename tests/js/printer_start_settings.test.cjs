...
function read(prime, valid = true, enabled = true) {
  const field = value => ({value, checkValidity: () => valid, reportValidity() {}});
  const ctx = vm.createContext({startPointPrimeEnabledInput: {checked: enabled}, earlyLayerZSpeedLimitInput: {checked: enabled}, earlyLayerSpeedLimitInput: {checked: enabled}, startPointPrimeInput: field(prime), earlyLayerSpeedInput: field('35'), earlyLayerZSpeedInput: field('2')});
...
test('saved form payload preserves zero prime and custom early-layer limits', () => {
  assert.deepEqual(read('0'), {start_point_prime_enabled: true, early_layer_z_speed_limit_enabled: true, early_layer_speed_limit_enabled: true, start_point_prime_mm: 0, early_layer_speed_mm_s: 35, early_layer_z_speed_mm_s: 2});
...
test('unchecked options preserve their editable numeric values in the save payload', () => {
  assert.deepEqual(read('0.03', true, false), {start_point_prime_enabled: false, early_layer_z_speed_limit_enabled: false, early_layer_speed_limit_enabled: false, start_point_prime_mm: 0.03, early_layer_speed_mm_s: 35, early_layer_z_speed_mm_s: 2});
});
