...
def test_lerobot_guided_device_bridge_is_separate_from_existing_gui(tmp_path: Path, monkeypatch: Any) -> None:
    cfg = copy.deepcopy(load_all_configs(resolve_path("configs")).get("lerobot", {}))
    cfg["device_memory_path"] = str(tmp_path / "memory" / "lerobot_device_ports.json")
    cfg["fake_dataset_root"] = str(tmp_path / "fake_datasets")
    cfg["fake_checkpoint_root"] = str(tmp_path / "fake_checkpoints")
    ...
    existing = client.get("/lerobot")
    assert existing.status_code == 200
    assert "Lerobot Framework GUI" in existing.text
    assert "ROBOTIS Guided Device Bridge" not in existing.text

    home = client.get("/")
    assert home.status_code == 200
    assert "Open LeRobot GUI" in home.text
    assert "Open Guided Device Bridge" in home.text
    assert "/device-bridge/lerobot" in home.text

    page = client.get("/device-bridge/lerobot")
    assert page.status_code == 200
    assert "ROBOTIS Guided Device Bridge" in page.text
    assert "Robot Readiness Board" in page.text
    assert "Policy Lifecycle" in page.text
    assert "Active Workflow Monitor" in page.text
    assert "guided-record-dataset-input" in page.text
    assert "guided-train-start" in page.text
    assert "guided-rollout-policy-input" in page.text
    assert "/static/lerobot_device_bridge.js" in page.text

    summary = client.get("/api/device-bridge/lerobot/summary").json()
    assert summary["ok"] is True
    assert summary["bridge"] == "lerobot_guided_device_bridge"
    assert summary["selected_profile_id"] == "robotis_omx_ai"
    readiness_ids = {item["id"] for item in summary["readiness"]}
    assert {"profile", "follower", "leader", "cameras", "policy", "safety", "active_workflow"}.issubset(readiness_ids)
    assert summary["workflow_actions"][0]["id"] == "setup_robot"
    assert any(action["id"] == "run_rollout" for action in summary["workflow_actions"])
    assert set(summary["workflow_diagnostics"]) >= {"record", "train", "rollout", "teleoperate", "manipulation"}
    assert isinstance(summary["job_cards"], list)
    assert isinstance(summary["sessions"], list)
    assert isinstance(summary["policies"], list)

    client.post("/api/lerobot/record/start", json={"mode": "test", "profile_id": "fake_omx_ai", "dataset_repo_id": "local/guided", "task_instruction": "Pick up the cylinder"})
    client.post("/api/lerobot/train/start", json={"mode": "test", "profile_id": "fake_omx_ai", "dataset_repo_id": "local/guided", "policy_type": "act", "steps": 20})
    client.post("/api/lerobot/rollout/start", json={"mode": "test", "profile_id": "fake_omx_ai", "policy_path": "fake://policy"})
    updated = client.get("/api/device-bridge/lerobot/summary").json()
    workflows = {card["workflow"] for card in updated["job_cards"]}
    assert {"record", "train", "rollout"}.issubset(workflows)
    assert updated["workflow_diagnostics"]["record"]["latest_status"]
