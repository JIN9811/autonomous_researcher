...
def test_specimen_pose_updates_red_cube_yaw_rotate_z(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert stage.prims[cube_path].attrs["xformOpOrder"].value == ["xformOp:rotateZ", "xformOp:scale", "xformOp:translate"]
...
def test_specimen_pose_uses_existing_orient_without_adding_second_rotation(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert result["ok"] is True
    assert result["xformOpOrder"] == ["xformOp:orient", "xformOp:scale", "xformOp:translate"]
...
def test_rgbd_render_backend_falls_back_to_step_async_inside_kit() -> None:
    ...
    result = backend._step_orchestrator(types.SimpleNamespace(orchestrator=orchestrator), {"rt_subframes": 2}, pending_key="sync")

    assert result == {"ok": True, "mode": "async_fallback"}
    assert orchestrator.sync_calls == [{"rt_subframes": 2, "delta_time": 0.0, "pause_timeline": False}]
    assert orchestrator.async_calls == [{"rt_subframes": 2, "delta_time": 0.0, "pause_timeline": False}]
...
def test_rgbd_render_backend_schedules_step_async_when_event_loop_is_running() -> None:
    ...
    async def _scenario() -> None:
        ...
        result = backend._step_orchestrator(types.SimpleNamespace(orchestrator=orchestrator), {"rt_subframes": 3}, pending_key="running-loop")
        ...
        assert orchestrator.async_calls == [{"rt_subframes": 3, "delta_time": 0.0, "pause_timeline": False}]
