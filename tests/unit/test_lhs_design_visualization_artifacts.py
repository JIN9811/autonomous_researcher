...
def _payload() -> dict[str, object]:
    ...
    return build_lhs_design_visualization(
        ...
        parameter_space={"cell_size_mm": [5.0, 6.0, 7.5, 10.0], "wall_thickness_mm": [0.2, 0.48]},
...
def test_write_lhs_artifacts_creates_separate_publication_bundle(tmp_path: Path) -> None:
    ...
    assert list(rows[0]) == [
        "index", "status", "candidate_id", "cell_size_mm", "wall_thickness_mm", "density_stratum"
    ]
    ...
    assert "Wall thickness (mm)" in svg
    assert "Wall thickness strata" in svg
