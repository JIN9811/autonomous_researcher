...
Knowledge now uses ontology-guided Markdown curation and scoped retrieval. The
Knowledge Graph/reconciliation path is retired; the Evolution records and
approval lifecycle described below are preserved. See the [current Knowledge
reference](knowledge_agent.md) for decision tools and storage contracts.
...
lineage is evidence for BO and scoped knowledge queries, not authority to approve or
...
memory/knowledge/markdown/<run>/<cycle>/<agent>/<record>/revision-*.md
```
...
- `POST /api/knowledge/markdown/query`
- `POST /api/knowledge/markdown/read`
...
- Markdown decision: selected scope, source citations, note receipts and explicit evidence gaps.
...
- Derived notes must cite inspected/read evidence and retain applicability.
- Terminal archive intake failure does not change the original device outcome.

## Historical File-Backed Verification Evidence

The evidence below predates graph retirement. It documents preserved Evolution
features, not a fresh run of every historical command. Current Knowledge
verification is in [Artifacts and Verification](knowledge_agent.md#artifacts-and-verification).
