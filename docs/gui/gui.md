...
- Current `/api/bridges` entries are graph metadata bridges only: Prusa, LeRobot, Windows PyAutoGUI, FEniCSx/CAE, and Camera/UTM. Bambu Lab X2D is exposed through the printer fleet layer (`/api/printer/fleet`) and `/api/printer/*` APIs, not through this registry.
