...
- The verified Bambu standalone path is recorded in `memory/bambu_autoejection.json.runtime_paths`: `/api/printer/autoejection-test -> mqtt_gcode_line -> device/<printer_serial>/request`. `home_after_standalone=false` is intentional; homing after standalone ejection is currently held, and unexpected `G28` inside the generated standalone tail is a blocker.
