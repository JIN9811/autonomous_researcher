...
- Device Workspaces panel with dedicated launchers for the selected 3DP Printer Bridge, Windows PyAutoGUI Bridge GUI, LeRobot / ROBOTIS GUI, BO Workspace, and CAE Analysis Workspace
- live loop timeline
- agent status
- device health
...
- The Bambu connection panel also renders a `Connection Confirmation` evidence board. It is read-only and derived from the saved connection plus the latest `/api/printer/status?mode=live` or `/api/printer/spc-readiness` response. It maps `BAMBU_LAN_MODE_NOT_CONFIRMED`, `BAMBU_DEVELOPER_MODE_NOT_CONFIRMED`, `BAMBU_FTPS_WRITE_FAILED`, `BAMBU_STORAGE_TRANSFER_PATH_NOT_VERIFIED`, and `BAMBU_HTTP_ARTIFACT_ROUTE_ACTIVE` into visible operator actions. The board must not infer upload readiness from form checkboxes alone.
  ...
  - Unless the request explicitly supplies `load_settings`, the backend creates a local no-skirt Bambu process profile copy under the slicing output folder and passes it through `--load-settings`/`--load-filaments`. The copy sets skirt loops, brim width/type, raft layers, and prime tower brim to off/zero. The original Bambu Studio presets are not modified.
...
- Bambu autoejection tests must not reuse the Prusa MK4S bed-sweep workflow. In the default Bambu profile the left/center/right controls generate standalone Bambu ejection artifacts or validation summaries with `motion_started=false` unless the explicit live standalone publish gate is passed. If that gate is passed, the standalone controls publish direct MQTT `gcode_line` through `/api/printer/autoejection-test` and do not use HTTP artifact route, FTPS upload, or MQTT `project_file`.
- The verified Bambu standalone path is recorded in `memory/bambu_autoejection.json.runtime_paths`: `/api/printer/autoejection-test -> mqtt_gcode_line -> device/20P6BJ642001425/request`. `home_after_standalone=false` is intentional; homing after standalone ejection is currently held, and unexpected `G28` inside the generated standalone tail is a blocker.
...
- When Pi0.5 is selected, the GUI switches defaults to `batch_size=32`, `steps=3000`, `num_workers=12`, `eval_freq=500`, `log_freq=5`, `save_freq=500`, `policy.chunk_size=50`, `policy.n_action_steps=50`, W&B local/offline logging, and seeds hardware-compatible fine-tuning args: `--policy.compile_model=false`, `--policy.gradient_checkpointing=true`, `--policy.dtype=bfloat16`, `--policy.freeze_vision_encoder=false`, and `--policy.train_expert_only=false`. The bridge allows Pi0.5 `num_workers` up to 20 for faster local data loading, while keeping the GUI default at 12 as the current local throughput/safety balance. The train video backend prefers `torchcodec` and falls back to `pyav` for live runs if the selected conda env cannot import TorchCodec. The bridge forces `compile_model=false` for Pi0.5 on this workstation because the current Triton/PTXAS stack rejects Blackwell `sm_121a` during Torch Inductor compilation. If a stale browser payload sends `wandb_enable=true` without a mode and no W&B API key is configured, the bridge forces `--wandb.mode=offline` instead of allowing a login-prompt failure. If `lerobot/pi05_base` is selected and local cache contains a compatible snapshot whose preprocessor no longer references `relative_actions_processor`, the bridge passes that local snapshot path to LeRobot instead of the stale HF ref.
