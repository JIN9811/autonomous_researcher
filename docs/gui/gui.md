...
- The Bambu connection panel also renders a `Connection Confirmation` evidence board. It is read-only and derived from the saved connection plus the latest `/api/printer/status?mode=live` or `/api/printer/spc-readiness` response. It maps `BAMBU_LAN_MODE_NOT_CONFIRMED`, `BAMBU_DEVELOPER_MODE_NOT_CONFIRMED`, `BAMBU_FTPS_WRITE_FAILED`, `BAMBU_STORAGE_TRANSFER_PATH_NOT_VERIFIED`, and `BAMBU_HTTP_ARTIFACT_ROUTE_ACTIVE` into visible operator actions. The board must not infer upload readiness from form checkboxes alone.
  ...
  - `Slice Bambu Artifact` calls `/api/printer/bambu-slice-artifact` with a real local `.stl` or `.3mf` path from the `Bambu Source STL / 3MF Path` field. The backend runs Bambu Studio CLI with `--slice 0 --arrange 1 --ensure-on-bed --outputdir ...`, records size/hash/stdout/stderr tails, and writes the resulting `.gcode`, `.3mf`, or `.gcode.3mf` path back into `Bambu Sliced Artifact Path`. This operation never uploads to the printer and never publishes MQTT start.
  - `Pre-start Check` calls `/api/printer/bambu-prestart-check`. It runs the same real backend path in sequence: Bambu Studio slicing if a source STL/3MF is present, HTTP artifact route export and fetch proof, guarded start gate, SPC readiness, and autoejection handoff evidence. It returns stage cards under `Bambu Pre-start Checklist` and always keeps `will_publish=false` / `published=false`; it is an output-before-start validation surface, not a print-start button.
...
- If the backend returns `autoejection_handoff`, the SPC Readiness next-action area must render a `Manipulation Agent handoff` card showing the consumer agent/tool and `motion_started=false`. This card is evidence display only; it must not start robot motion.
- `readiness_levels` separate connection, sliced-artifact transfer, operator/Guardian approval, MQTT publish command, and autoejection loop readiness. `technical_ready_for_start=true` means the printer/transfer/start-command technical gates are clear, even if operator/Guardian approval or dry-run policy still prevents publish.
- `Start Gate Check` and `Publish Start` are intentionally separate. `Start Gate Check` calls `/api/printer/start-gate` and never publishes. `Publish Start` calls `/api/printer/start-publish` after a browser confirmation; backend still requires operator confirmation, Guardian approval, `dry_run=false`, a valid `project_file` draft, live preprint gate readiness, and device `can_start_print=true` before sending MQTT.
- The 3DP GUI must not hardcode operator or Guardian approval. `Operator confirmed`, `Guardian approved`, and `dry-run / no publish` are visible start-gate controls, and their checkbox values are passed unchanged to Start Gate, SPC Readiness, and Publish Start requests. Default state is dry-run enabled with both approvals unchecked.
- The 3DP Bambu Device Screen consumes backend `device_screen` panels, not ad-hoc frontend guesses:
  ...
  - `camera_panel`: MQTT-reported live preview metadata plus explicit Bambu LAN video probe/proxy state. `Video Status` calls `/api/printer/video-status`, probes RTSPS `:322` before JPEG `:6000`, and shows blockers such as `BAMBU_VIDEO_PORT_UNREACHABLE` or `BAMBU_VIDEO_PROXY_FFMPEG_MISSING`. If `ffmpeg` is installed and the port is reachable, `/api/printer/video-stream.mjpeg` exposes a browser MJPEG proxy without echoing the LAN access code.
...
- The printer runtime card must show storage readiness plus upload/start status. For Bambu, FTPS write readiness, HTTP artifact route readiness, start-command draft, operator confirmation, Guardian approval, and autoejection evidence are separate gates.
...
- The intended Bambu pre-start operator path is: choose active Bambu profile, save/confirm connection, enter a source STL/3MF, click `Pre-start Check`, inspect the stage cards and generated artifact URL/hash, then use `Publish Start` only after dry-run is disabled and operator/Guardian approvals are explicitly checked. The manual `Slice Bambu Artifact`, `Prepare HTTP Artifact`, and `Start Gate Check` buttons remain available for debugging each step separately.
...
- `http_artifact_ready_not_started` means the sliced artifact URL and guarded MQTT `project_file` draft are prepared. In this state `technical_ready_for_start` and `device_screen.actions.can_start_print` may be true because the technical transfer/start-command gate is clear, but `Publish Start` still requires dry-run off, operator confirmation, Guardian approval, safe printer state, and backend start gate success before any MQTT command is sent.
- Bambu autoejection evidence is configured through `/api/printer/autoejection-config` and stored locally in `memory/bambu_autoejection.json`. The Live GUI must not mark the autonomous loop ready from the print-profile checkbox alone; provider, verified routine id, pre-eject vision profile, and post-eject vision profile are all required.
- Bambu autoejection is a provider handoff, not a physical ejection command. In addition to `memory/bambu_autoejection.json`, the backend must verify a saved Manipulation Agent consumer profile at `memory/manipulation_agent_bridge.json`. The consumer is ready only when it names the 3DP-to-UTM task (`task_id=transfer_to_utm`), uses a rollout-capable strategy such as `pi05_lerobot_policy`, and points to an existing local policy path or a configured policy repository.
- `/api/printer/autoejection-status`, `/api/printer/autoejection-config`, `/api/printer/autoejection-test`, `/api/printer/spc-readiness`, and `/api/printer/bambu-prestart-check` all expose `consumer_readiness`. If provider evidence is configured but the Manipulation Agent consumer is not ready, Bambu `Check Handoff` controls stay blocked and the autonomous loop remains `attention`, even though the provider routine fields are present.
- The 3DP GUI may offer `Fill Manipulation Handoff Defaults` for the Bambu autoejection fields, but this button only fills local form values. It must not write `memory/bambu_autoejection.json`, mark the gate ready, or run motion until the operator explicitly clicks `Save Autoejection Gate` after verifying the manipulation routine and vision profiles.
- Bambu autoejection tests must not reuse the Prusa MK4S bed-sweep G-code workflow. In the default Bambu profile the UI labels the left/center/right controls as `Check Handoff`, and `/api/printer/autoejection-test` returns a `bambu_autoejection_provider_handoff.v1` packet for the configured external provider/Manipulation Agent with `motion_started=false`. After saving Bambu autoejection evidence, the GUI immediately rereads live printer status and autoejection status so the Control Gate/SPC screen does not show stale readiness.
- The Bambu autoejection handoff packet must preserve the execution boundary: `recommended_consumer_agent=ManipulationAgent`, `next_tool=lerobot.manipulation-agent.run`, `requires_guardian_approval=true`, `requires_operator_confirmation=true`, `motion_started=false`, and `dry_run_only=true`. The 3DP GUI status line and Live GUI Specimen runtime report must show these fields plus `consumer_readiness` so operators do not mistake a provider handoff for physical ejection.
...
- Module Designer accepts a Python file and sends it through the active backend's `module_designer` route for ATR protocol conversion. The active route primary model is tried first, the active route model fallback is tried second, and if `backend.fallback: openai` is configured the OpenAI API model is tried last. The result is saved as `graphs/modules/<module_id>/handler.py` plus `module.yaml` metadata, category, tool allowlist, internal steps, and version history. The generated handler remains pending until it is registered in the allowlisted runtime handler registry; the GUI never executes arbitrary uploaded Python directly.
