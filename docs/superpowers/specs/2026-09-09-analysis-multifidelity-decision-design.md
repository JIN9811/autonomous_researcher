...
**기구 우선 검증 보완:** remeshed baseline은 유지하고, 격자의 하중 감소를 재료
연화로 간주하지 않는다. `analysis_mechanism_assessment.v1`로 수치 완료·물성 근거·
변형 양상을 분리한다. 현재 연화 탐색은 별도 출력 재료시험 또는 적용성 검토된
문헌 근거와 참조가 있는 변형 비교가 있어야 진입 가능하다. 입력은
`analysis_improvement.mechanism_evidence`로 받고 기존 런 입력 저장소에 동결한다.
해시는 출처 무결성을 확인할 뿐 물리적 타당성을 인증하지 않는다.

LLM은 허용된 메쉬 검증 툴 또는 비구동 근거 요청을 선택한다. 부분 계산은 개별
job의 conclude와 무관하게 재료 탐색을 중단시키고 수치 진단을 요구한다.
근거 없는 물성 추정으로 경계조건·좌굴·층간 손상·자기접촉 누락을 보상하지 않는다.
Explicit 준정적/손상 정규화/자기접촉은 현재 등록된 계산 경로가 아니므로 실행했다고
기록하지 않는다. 향후 도입 시 운동·내부·인공 에너지, 속도·증분·메쉬 민감도와
변형 양상을 검증하고 독립 시편으로 순방향 예측을 평가한다. 이 보완은 아래 초기
연화 가설의 실행 자격을 제한하며 새로운 PLA 물성의 검증 완료를 의미하지 않는다.

2026-09-09 추가 승인: 시험 곡선의 점들을 물성표로 복사하지 않고, 특징을 설명하는
소수의 구성방정식 파라미터를 역해석으로 식별한다. 단일 시편으로 보정 연구는
실행할 수 있지만, 보정에 쓰지 않은 별도 실험에서 파라미터를 고정한 순방향 예측을
검증하기 전에는 재료 모델을 승격하지 않는다. 기존 비차단 FEM 진입점의 명시적
`policy.calibration`으로 실행하며 foreground Analysis→BO와 장비 경로는 유지한다.
첫 구현은 지수형 항복 후 유동응력 가설과 유한 좌표 탐색이다. 초기값과 bounds는
실측 물성이 아닌 탐색 가정으로 기록하고, 국소 연화의 메쉬 의존성은 별도 검증한다.
전체 목표 구간을 계산하지 못한 후보, proxy 결과, 동일 적분값만 맞는 잘못된 곡선은
...
최근 후보 이력을 전달하고, 중복 요청 본문과 원본 필드 배열은 아티팩트에 보존한다.
백엔드별 검증은 폴백 없이 모델을 고정하며 서버 수명주기나 전역 설정을 바꾸지 않는다.
API와 Gemma 31B 각각 6개 시나리오를 통과했다. 실제 LLM + 보존 native 결과 재생,
실제 LLM + 수치 fixture를 이용한 2후보 개선 루프 검증이며, 새 native 계산 완료나
독립 실험 예측 성공을 대신하는 근거로 사용하지 않는다.
