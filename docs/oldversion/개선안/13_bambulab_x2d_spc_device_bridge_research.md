...
artifact=<home>/autonomous_researcher/artifacts/bambu_sliced/qa_specimen.gcode.3mf
...
```text
docs/oldversion/개선안/reference_assets/bambulab_device_screen_20260614.png
```
...
- User supplied Bambu Device reference screenshot: `docs/oldversion/개선안/reference_assets/bambulab_device_screen_20260614.png`
...
- `device_bridges/bambu_bridge.py`
  ...
  - `BambuSlicerConfig.resolved_payload()`가 Bambu Studio CLI를 `BAMBU_STUDIO_EXECUTABLE` env var, configured wrapper path, `PATH`의 `bambu-studio` 순서로 해석한다. 현재 Spark workstation에서는 `<home>/.local/bin/bambu-studio`가 감지된다.
...
can_upload: false
can_start_print: false
Bambu Studio CLI: <home>/.local/bin/bambu-studio detected via PATH
