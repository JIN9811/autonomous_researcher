...
```text
<home>/external_repos/UTM/scripts/start_utm_vision_stack.sh
<home>/external_repos/UTM/src/compression_tester_monitor/launch/camera_rect.launch.py
<home>/external_repos/UTM/src/compression_tester_monitor/launch/green_dot_monitor.launch.py
<home>/external_repos/UTM/scripts/yolo.sh
```
...
2. UTM repo 기본 경로는 `<home>/yolo_ros_ws/UTM_VISION`이다. ATR에서는 `configs/devices.yaml` + environment override 방식으로 바꿔야 한다.
...
```text
<home>/external_repos/UTM
<home>/yolo_ros_ws/UTM_VISION
...
<home>/external_repos/UTM/src/compression_tester_monitor/launch/green_dot_monitor.launch.py
<home>/external_repos/UTM/scripts/yolo.sh
<home>/external_repos/UTM/src/compression_tester_monitor/README.md
```
...
- expected graph는 반드시 `<home>/external_repos/UTM`의 current launch/script/docs에서 유도한다. UTM repo가 업데이트되어 launch topic/remapping이 바뀌면 expected graph도 그 파일을 다시 읽어 갱신한다.
