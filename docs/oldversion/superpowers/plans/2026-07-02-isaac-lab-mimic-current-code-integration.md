...
**Tech Stack:** FastAPI, Pydantic, LeRobot bridge, Isaac Sim `<home>/IsaacSim/python.sh`, Isaac Lab `release/3.0.0-beta2`, `ManagerBasedRLMimicEnv`, Isaac Lab Mimic, robomimic BC, HDF5, Replicator/Isaac Lab event randomization, pytest.
...
def main() -> None:
    register()
    isaac_lab = Path(sys.argv[sys.argv.index("--isaac-lab-path") + 1]) if "--isaac-lab-path" in sys.argv else Path("<home>/IsaacLab")
...
def main() -> None:
    ...
    parser.add_argument("--isaac-lab-path", default="<home>/IsaacLab")
    parser.add_argument("--isaac-sim-python", default="<home>/IsaacSim/python.sh")
