...
```text
<home>/.cache/huggingface/lerobot/local-pi05-v30/jin-20260703-1
```

**Selected source HDF5:**

```text
<home>/.cache/huggingface/lerobot/local-pi05-v30/jin-20260703-1/sidecar/isaac_lab_synthetic/latest/hdf5/exported_successful_real_episodes.hdf5
```
...
```bash
PYTHONPATH=<home>/autonomous_researcher \
<home>/miniconda3/envs/lerobot/bin/python \
<home>/autonomous_researcher/scripts/lerobot_isaac_lab_validate.py \
  --action build-synthetic \
  --dataset <home>/.cache/huggingface/lerobot/local-pi05-v30/jin-20260703-1 \
  --mode test \
  --max-source-frames 0 \
...
```bash
PYTHONPATH=<home>/autonomous_researcher \
<home>/IsaacLab/isaaclab.sh -p <home>/IsaacLab/scripts/tools/replay_demos.py \
...
```text
source HDF5:
<home>/.cache/huggingface/lerobot/jin/20260703_1/sidecar/isaac_lab_synthetic/latest/hdf5/exported_successful_real_episodes.hdf5

command:
PYTHONPATH=<home>/autonomous_researcher \
<home>/IsaacLab/isaaclab.sh -p <home>/IsaacLab/scripts/tools/replay_demos.py \
  --task ATR-Robotis-OMX-PickPlace-Physical-Mimic-v0 \
  --dataset_file <home>/.cache/huggingface/lerobot/jin/20260703_1/sidecar/isaac_lab_synthetic/latest/hdf5/exported_successful_real_episodes.hdf5 \
  --select_episodes 0 \
  --validate_success_rate \
  --reset_sim_buffer_each_episode \
  --enable_cameras \
...
```text
<home>/IsaacLab/_isaac_sim/python.sh -m pytest -q \
...
```text
dataset:
<home>/.cache/huggingface/lerobot/jin/20260703_1
...
```text
input:
<home>/.cache/huggingface/lerobot/jin/20260703_1/sidecar/isaac_lab_synthetic/latest/hdf5/source_real_success_annotated.hdf5
...
device_bridges/isaac_lab_synthetic.py official backend now passes:
  --annotate-script <home>/IsaacLab/scripts/imitation_learning/isaaclab_mimic/annotate_demos.py
  --annotation-mode auto
  --generate-script <home>/IsaacLab/scripts/imitation_learning/isaaclab_mimic/generate_dataset.py
...
```text
run:
<home>/autonomous_researcher/runs/isaac_lab_official_mimic_gui_path_20260708T144404
...
```text
run:
<home>/autonomous_researcher/runs/isaac_lab_official_mimic_multi_ep_gate_20260708T150338
...
<home>/.cache/huggingface/lerobot/jin/20260703_1/sidecar/isaac_lab_synthetic/latest
...
output root:
<home>/autonomous_researcher/runs/isaac_lab_gui_api_smoke_20260708_191502
...
isaac_sim_python=<home>/IsaacLab/isaaclab.sh
```
