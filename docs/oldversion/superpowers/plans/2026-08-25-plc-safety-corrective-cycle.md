...
**Spec:** `docs/oldversion/superpowers/specs/2026-08-24-plc-safety-bridge-design.md`
...
- Existing changes in `<home>/autonomous_researcher` must not be overwritten or reverted.
...
Run: `<home>/autonomous_researcher/.venv/bin/python -m pytest -q tests/unit/test_controller_plc_safety.py -k 'result_status or physical_command_active'`
...
Run: `<home>/autonomous_researcher/.venv/bin/python -m pytest -q tests/unit/test_plc_bridge_service.py -k 'reestop or acknowledged or transaction_preserved'`
...
Run: `<home>/autonomous_researcher/.venv/bin/python -m pytest -q tests/unit/test_controller_plc_safety.py tests/unit/test_plc_bridge_service.py`
...
Run: `<home>/autonomous_researcher/.venv/bin/python -m pytest -q tests/integration/test_plc_bridge_api.py tests/integration/test_plc_safety_e2e.py -k 'recovery_route or degraded_sync or preflight_handshake'`
...
Run: `<home>/autonomous_researcher/.venv/bin/python -m pytest -q tests/integration/test_plc_bridge_api.py tests/integration/test_plc_safety_e2e.py`
...
Run: `<home>/autonomous_researcher/.venv/bin/python -m pytest -q tests/integration/test_plc_bridge_api.py tests/ui/plc_workspace_browser_audit.py`

- [x] **Step 4: Document runtime checkout diagnosis**

Record that a server whose `/proc/<pid>/cwd` is `<home>/autonomous_researcher` will not expose worktree-only routes until the reviewed commits are integrated or that worktree is started explicitly. Do not instruct operators to overwrite a dirty checkout.
...
**Files:**
- Modify: `docs/oldversion/superpowers/plans/2026-08-25-plc-safety-corrective-cycle.md`
...
Do not merge into `<home>/autonomous_researcher`, restart its server, or operate physical PLC registers without explicit approval after review passes.
...
- Serving checkout `<home>/autonomous_researcher`: intentionally not modified or restarted because it contains unrelated user changes
