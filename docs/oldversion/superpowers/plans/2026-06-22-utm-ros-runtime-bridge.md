...
## Objective
Add a production-ready UTM ROS Vision runtime bridge to ATR without replacing the existing ATR architecture. The RQT-like graph must follow the cloned UTM repository at `<home>/external_repos/UTM` as the source of truth.
...
3. Extend `configs/devices.yaml` with `devices.utm_vision_runtime` defaulting to `<home>/external_repos/UTM`.
