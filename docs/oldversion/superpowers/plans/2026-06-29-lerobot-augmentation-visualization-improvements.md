...
- `<home>/IsaacLab` exists locally at `v3.0.0-beta2`. The synthetic branch must treat Isaac Lab as a pinned external runtime with explicit Isaac Sim compatibility checks, not as an unversioned helper script directory.
...
```json
{
  ...
  "status": "READY_FOR_TRAINING",
  "dataset_path": "<home>/autonomous_researcher/data/example",
  "output_root": "<home>/autonomous_researcher/data/example/sidecar/isaac_lab_synthetic/runs/20260701_120000_a1b2c3",
  "pipeline_mode": "isaac_lab_replicator",
...
- Local `<home>/IsaacLab` is reported when present.
