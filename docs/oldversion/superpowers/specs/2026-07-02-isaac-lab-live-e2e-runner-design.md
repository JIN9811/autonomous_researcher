...
```bash
<home>/miniconda3/envs/lerobot/bin/python scripts/lerobot_isaac_lab_e2e_smoke.py \
  --mode live \
  --dataset-path <selected_dataset> \
  --isaac-lab-path <home>/IsaacLab \
  --isaac-sim-python <home>/IsaacSim/python.sh \
  --trials 3 \
  --num-envs 2 \
  --episodes 3 \
  --episode-s 10 \
...
```text
<home>/.cache/huggingface/lerobot/local-pi05-v30/jin-live-10s3-viz-20260702t213455-0900
```
