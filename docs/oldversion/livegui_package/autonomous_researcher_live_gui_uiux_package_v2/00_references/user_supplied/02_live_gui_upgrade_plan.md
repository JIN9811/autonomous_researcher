...
- 스타일 레퍼런스와 구현 asset은 `docs/oldversion/livegui_package/autonomous_researcher_live_gui_uiux_package_v2`에 제공되어 있다.
...
### 2.4 `docs/oldversion/livegui_package` 통합 기준

새로 추가된 `docs/oldversion/livegui_package/autonomous_researcher_live_gui_uiux_package_v2`는 단순 참고 이미지가 아니라 구현에 바로 분해해서 쓸 수 있는 UI/UX package다. 기존 `/live` 구조를 대체하지 않고, 현재 `planning.html`, `planning.js`, `styles.css`, `app/controller.py`, `app/main.py`에 점진 이식한다.
...
이제 임시 기준이 아니라 `docs/oldversion/livegui_package`의 design token과 generated full-screen reference를 기준으로 한다. 다만 현재 코드의 기존 DOM ID/API/guard는 유지한다.
...
`docs/oldversion/livegui_package/autonomous_researcher_live_gui_uiux_package_v2/06_implementation_guides/first_patch_checklist.md`를 그대로 작업 checklist로 사용한다.
...
12. `docs/oldversion/livegui_package` reference image와 crop 기준으로 1920x1080 visual audit을 통과한다.
...
`docs/oldversion/livegui_package`가 제공된 상태이므로 다음 순서로 바로 구현한다.
