...
See the [Design](../agents/design_agent.md#editable-runtime-ide-structure),
...
| Control | Draft behavior | Runtime effect |
|---|---|---|
...
## Virtual-Device Execution Contract

`virtual_bridge` with all resolved devices virtual means normal agent decisions,
registered tool calls and orchestration handoffs, with equipment I/O simulated.
The application defaults to real LLM calls in TEST. An explicitly configured
offline test harness can replace model responses; its timing is not an LLM benchmark.

Vision retains tool selection and evidence review, Manipulation uses existing
virtual LeRobot sessions, and Equipment runs its simulated workflow and writes
the CSV consumed by Analysis. Synthetic evidence is labeled as such and cannot
