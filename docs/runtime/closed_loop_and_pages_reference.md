...
- **CAE loop**: Analysis Agent가 UTM geometry/material/loading에서 검증된 CAE payload를 만들고 `cae.run_static_analysis`를 호출한다. 현재 TEST loop는 `device_bridges/cae_bridge.py`의 deterministic equivalent path를 쓰고, 실제 CalculiX runner는 CAE bridge의 live solver 경로에서 확장한다.
