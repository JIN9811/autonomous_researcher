# LeRobot Dataset, Policy, and Rollout Naming Rules

Scope: LeRobot / ROBOTIS teleoperation, recording, training, policy rollout, and Manipulation Agent integration.

## Naming Contract

Use three separate names for the three separate artifacts:

- Training dataset: human demonstration data recorded by teleoperation.
- Policy: trained model checkpoint or Hub policy repo.
- Rollout/evaluation dataset: robot execution data generated by a loaded policy.

Example:

```text
training dataset repo_id:  jin/pick_and_place_cube
policy repo_id:            jin/pick_and_place_cube_act
rollout dataset repo_id:   jin/eval_pick_and_place_cube_rollout_20260508T131538
```

## `eval_` Rule

`eval_` belongs on rollout/evaluation output datasets, not on training datasets.

Rules:

- Do not prefix teleoperation training datasets with `eval_`.
- Do not prefix policy repo IDs with `eval_`.
- Always prefix policy-generated rollout datasets with `eval_`.
- If an operator, GUI, or Manipulation Agent sends `dataset_repo_id=jin/pick_and_place_cube_rollout`, the bridge must execute LeRobot with `dataset_repo_id=jin/eval_pick_and_place_cube_rollout`.
- If the final repo name already starts with `eval_`, keep it unchanged.

Reason:

The installed LeRobot runtime uses `lerobot-record` for policy-controlled evaluation when a policy is provided. Its safety check requires the dataset name to begin with `eval_` to prevent policy-generated evaluation data from being confused with human demonstration training data.

## Local Path Rule

For rollout on this workstation, `dataset.root` is the final dataset directory, not only the parent cache directory.

Example command shape:

```text
--dataset.repo_id=jin/eval_pick_and_place_cube_rollout_20260508T131538
--dataset.root=/home/jin/.cache/huggingface/lerobot/jin/eval_pick_and_place_cube_rollout_20260508T131538
```

This matches the currently installed LeRobot implementation, where `LeRobotDataset.create(..., root=<path>)` creates the dataset directly at `<path>`.

For live training, path resolution is stricter:

- If `dataset_path` is selected, it must resolve under an allowed local root and
  must contain a completed LeRobot dataset.
- If only `dataset_repo_id` is selected, the bridge searches under
  `dataset_root` and may pick the newest completed timestamped recording that
  matches the same repo prefix.
- Fresh training runs do not reuse an existing `output_dir`; the bridge creates
  a timestamp-suffixed output directory.
- Resume training keeps the selected `output_dir` but requires an existing
  checkpoint `train_config.json`.

For live rollout/inference, policy path resolution is also normalized by the
bridge:

- A selected output folder, a `checkpoints/last/pretrained_model` folder, or a
  recognized model file such as `model.safetensors`, `policy.ckpt`, `*.pt`,
  `*.pth`, or `*.bin` is resolved to the executable pretrained policy path.
- GUI, CUI, and Manipulation Agent should pass the operator's selected value;
  they must not duplicate checkpoint-directory search logic.

## RealSense Camera Preflight Rule

Camera path handling is part of rollout/record/teleop execution, not training.

Rules:

- `top` and `wrist` RealSense cameras are saved by SDK serial/name when
  `backend=intelrealsense`.
- Current Spark workstation defaults are `top=341522300873` for D455F and
  `wrist=352122273019` for D405.
- Command generation must preserve role-specific RealSense stream settings:
  `top/D455F -> color_format=rgb8`, `wrist/D405 -> color_format=bgr8`, both
  with `warmup_s>=20`, `use_depth=true`, and the saved FPS/resolution.
- Device Port Setup must not save top/D455F as wrist when D405 is absent. Missing
  D405 is represented as `LEROBOT_REALSENSE_ROLE_CAMERA_NOT_FOUND` during
  detect/save or `LEROBOT_REALSENSE_CAMERA_UNAVAILABLE` during live startup.
- Before live teleoperation, recording, or rollout starts with
  `camera_enabled=true`, the bridge enumerates visible RealSense devices from
  the LeRobot conda environment.
- Missing saved serials block before launching LeRobot with
  `LEROBOT_REALSENSE_CAMERA_UNAVAILABLE`.
- Do not fall back to `/dev/video*` or OpenCV for RealSense policy
  observations. Restore USB/SDK visibility and rerun Device Port Setup if
  necessary.

## RealSense Raw Depth Sidecar Rule

LeRobot policy compatibility and metric depth preservation are separate
artifacts.

Rules:

- Keep `observation.images.top_depth` and `observation.images.wrist_depth` as
  normal LeRobot image/video features. They are 8-bit 3-channel depth
  visualizations for the default `rgbd_sidecar` path. When
  `raw_depth_adapter` is selected, the patched LeRobot dataset loader keeps
  the same feature keys but replaces their tensors at training time with
  tensors reconstructed from the 16-bit raw sidecar.
- When live recording uses saved RealSense cameras with `use_depth=true`, the
  bridge must pass `ATR_LEROBOT_RAW_DEPTH_DIR=<dataset>/sidecar/depth_raw`,
  `ATR_LEROBOT_RAW_DEPTH_CAMERA_KEYS=<comma-separated keys>`, and
  `ATR_LEROBOT_RAW_DEPTH_FORMAT=png16` into the LeRobot subprocess.
- The bridge must also pass the transform contract used by both recording and
  rollout camera runtime: `ATR_LEROBOT_DEPTH_ALIGNED_TO=color`,
  `ATR_LEROBOT_DEPTH_SCALE_M_PER_UNIT=0.001`,
  `ATR_LEROBOT_DEPTH_CLIP_MIN_MM=0.0`, and
  `ATR_LEROBOT_DEPTH_CLIP_MAX_MM=2000.0` unless the operator deliberately
  changes `configs/lerobot.yaml`.
- LeRobot RealSense camera configs generated by the bridge must include
  `align_depth_to_color`, `depth_scale_m_per_unit`, `depth_clip_min_mm`, and
  `depth_clip_max_mm` so the command preview and the runtime process share the
  same RGB-D contract.
- The OMX follower writes one 16-bit PNG per frame under
  `sidecar/depth_raw/<camera_key>/frame_000000.png`,
  `frame_000001.png`, and so on. Existing files are not overwritten; the writer
  continues after the highest existing frame index for that camera key.
- The OMX follower writes
  `sidecar/depth_raw/transform_manifest.json` once per recording root. This
  manifest records alignment target, metric depth scale, raw encoding, and
  visual-depth clipping range for later training/inference verification.
- Raw sidecars preserve RealSense Z16 `uint16` depth values. In
  `rgbd_sidecar` they are evidence and audit material. In `raw_depth_adapter`
  they are active training input: the dataset loader reads
  `transform_manifest.json`, loads `frame_*.png`, applies metric scale and
  fixed clipping, resizes to the existing depth feature shape, and overwrites
  the corresponding `observation.images.<camera>_depth` tensor.

## Manual-Stop Rollout Rule

Policy rollout should run until the operator stops it unless a duration is explicitly requested.

Rules:

- Empty GUI rollout duration means `continuous_rollout=true`.
- `continuous_rollout=true` maps to one long LeRobot episode with `episode_s=86400.0` and `num_episodes=1`.
- Pressing `Stop Rollout` terminates the active LeRobot rollout process group.
- If the GUI/user provides a rollout duration, set `continuous_rollout=false`, `episode_s=<duration>`, and `num_episodes=1`.
- When rollout uses a local checkpoint with `--policy.path=<checkpoint_dir>`,
  do not also pass `--policy.type=<type>`. The installed LeRobot parser treats
  those options as mutually exclusive and aborts before robot control starts.

This is a pragmatic workaround because the installed LeRobot `lerobot-record --policy.path=...` path is episode-duration based and does not expose a native infinite policy-control mode.

## Safe Rollout Action Clamp

Rollout should enable per-step target clamping by default.

Rules:

- GUI default: `rollout_action_clamp=true`.
- GUI default limit: `rollout_max_relative_target=5`.
- Bridge command when enabled: `--robot.max_relative_target=5`.
- If disabled, the bridge omits `--robot.max_relative_target` and LeRobot falls back to the robot config default, currently `None` for `omx_follower`.

Purpose:

The clamp limits abrupt target jumps produced by a policy rollout. It does not fix a bad policy or camera/pose mismatch, but it reduces sudden large per-step direction changes and makes early rollout validation safer.

## Pi0.5 Training Runtime

Pi0.5 support is intentionally isolated from the ROBOTIS OMX live runtime.

Rules:

- Default robot runtime: `/home/jin/lerobot` in conda env `lerobot`.
- Pi0.5 training runtime: `/home/jin/lerobot_pi05` in conda env `lerobot-pi05-torch211`.
- Pi0.5 Hugging Face cache: `HF_HOME=/home/jin/.cache/huggingface_pi05`, `HF_HUB_CACHE=/home/jin/.cache/huggingface_pi05/hub`, `HF_HUB_DISABLE_XET=1`.
- TorchCodec-first video decoding: training commands prefer `--dataset.video_backend=torchcodec`; live runs fall back to `pyav` if the selected conda environment does not expose `torchcodec`.
- GUI policy type: `pi05`.
- Default HF source policy: `lerobot/pi05_base`.
- Bridge command for Pi0.5 training uses `conda run --no-capture-output -n lerobot-pi05-torch211 lerobot-train`.
- The Pi0.5 source model is passed as `--policy.pretrained_path=lerobot/pi05_base`; this differs from the older LeRobot 0.3.x `--policy.path=<repo-or-dir>` path used by some non-Pi0.5 fine-tuning flows.
- Pi0.5 live training requires LeRobot dataset format `v3.0`. If the selected local training dataset is `v2.1`, the bridge must not mutate the original ROBOTIS/LeRobot recording. It must copy the dataset to `~/.cache/huggingface/lerobot/local-pi05-v30/<dataset-slug>`, run the v2.1 -> v3.0 converter on that generated copy, then train against the converted repo id/path.
- Generated Pi0.5 v3.0 datasets are excluded from generic "latest local dataset" selection unless the operator explicitly selects the `local-pi05-v30/...` repo id. This prevents ACT training from accidentally picking a Pi0.5-only converted dataset.

Torch 2.11/TorchCodec setup command:

```bash
conda run -n lerobot-pi05-torch211 python -m pip install --upgrade --force-reinstall torch==2.11.0 torchvision==0.26.0 torchcodec==0.13.0
conda run -n lerobot-pi05-torch211 python -m pip install --force-reinstall fsspec==2025.9.0 setuptools==80.10.2
```

Prefetch command:

```bash
mkdir -p /home/jin/.cache/huggingface_pi05
HF_HOME=/home/jin/.cache/huggingface_pi05 HF_HUB_CACHE=/home/jin/.cache/huggingface_pi05/hub HF_HUB_DISABLE_XET=1 conda run -n lerobot-pi05-torch211 hf download lerobot/pi05_base --max-workers 1
```

Recommended additional train args for Pi0.5 on this workstation. Quantile stats are generated automatically for `observation.state` and `action` when missing; do not override Pi0.5 normalization unless intentionally debugging a dataset.

```text
--policy.compile_model=true
--policy.gradient_checkpointing=true
--policy.dtype=bfloat16
--policy.freeze_vision_encoder=false
--policy.train_expert_only=false
```

Manual local conversion command shape:

```bash
conda run --no-capture-output -n lerobot-pi05-torch211 python -m lerobot.datasets.v30.convert_dataset_v21_to_v30 \
  --repo-id=local-pi05-v30/jin-record-test-20260512t063639z \
  --root=/home/jin/.cache/huggingface/lerobot \
  --push-to-hub=false \
  --force-conversion
```

`--root` is the LeRobot dataset cache parent. The converter appends `--repo-id` internally, so do not pass the final dataset directory as `--root`.

Policy-specific GUI defaults:

- ACT follows the Hugging Face LeRobot train defaults and ACT config: `batch_size=8`, `steps=100000`, `num_workers=4`, `eval_freq=20000`, `log_freq=200`, `save_freq=20000`, `policy.n_obs_steps=1`, `policy.chunk_size=100`, `policy.n_action_steps=100`.
- Pi0.5 follows the Hugging Face Pi0.5 fine-tuning example and the local workstation profile: `batch_size=16`, `steps=3000`, `num_workers=12`, `eval_freq=500`, `log_freq=50`, `save_freq=500` by default, `policy.n_obs_steps=1`, `policy.chunk_size=50`, `policy.n_action_steps=50`, plus `compile_model=true`, `gradient_checkpointing=true`, `dtype=bfloat16`, `freeze_vision_encoder=false`, and `train_expert_only=false`. The backend hard limit for Pi0.5 `num_workers` is 20 on this workstation.
- X-VLA is available as an additive low-data VLA candidate under policy type `xvla`. It runs through the TorchCodec-ready LeRobot conda environment `lerobot-pi05-torch211`, not the generic `lerobot` environment. The bridge default source is `lerobot/xvla-base`, passed as `--policy.path=lerobot/xvla-base`; the GUI default is `steps=20000`, `batch_size=8`, and `num_workers=4`.
- X-VLA training args seeded by the bridge/GUI are `--policy.dtype=bfloat16`, `--policy.action_mode=auto`, `--policy.freeze_vision_encoder=false`, `--policy.freeze_language_encoder=false`, `--policy.train_policy_transformer=true`, and `--policy.train_soft_prompts=true`. Operators may still add other safe one-arg-per-line CLI flags through Additional Train CLI Args.
- X-VLA rollout uses the generic LeRobot rollout command path in conda env `lerobot-pi05-torch211`; local checkpoint execution is driven by `--policy.path=<checkpoint_dir>` and ACT temporal-ensemble / Pi0.5 RTC flags are intentionally omitted.
- SmolVLA is available as an additive LeRobot VLA candidate under policy type `smolvla`. It uses the TorchCodec-ready LeRobot conda environment `lerobot-pi05-torch211`, not the generic `lerobot` environment. The bridge default source is `lerobot/smolvla_base`, passed as `--policy.path=lerobot/smolvla_base`; the GUI default is `steps=20000`, `batch_size=8`, `num_workers=4`, `policy.n_obs_steps=1`, `policy.chunk_size=50`, and `policy.n_action_steps=50`.
- SmolVLA training args seeded by the bridge/GUI are `--policy.freeze_vision_encoder=true`, `--policy.train_expert_only=true`, and `--policy.train_state_proj=true`. Operators must install the LeRobot `smolvla` extra and prefetch both `lerobot/smolvla_base` and `HuggingFaceTB/SmolVLM2-500M-Video-Instruct` for offline-first startup.
- SmolVLA rollout uses the generic LeRobot rollout command path in conda env `lerobot-pi05-torch211`; local checkpoint execution is driven by `--policy.path=<checkpoint_dir>` and ACT temporal-ensemble / Pi0.5 RTC flags are intentionally omitted.
- Official references: https://huggingface.co/docs/lerobot/act, https://huggingface.co/docs/lerobot/pi05, https://huggingface.co/docs/lerobot/xvla, and the local LeRobot `policies/smolvla` module docs.

## ACT Temporal Ensemble Filter

Rollout should enable ACT temporal ensembling by default for smoother policy actions.

Rules:

- GUI default: `rollout_temporal_ensemble=true`.
- GUI default coefficient: `rollout_temporal_ensemble_coeff=0.01`.
- Bridge command when enabled: `--policy.temporal_ensemble_coeff=0.01`.
- Bridge must also add `--policy.n_action_steps=1`, because the installed LeRobot ACT config requires `n_action_steps=1` when temporal ensembling is enabled.
- If disabled, the bridge must omit both `--policy.temporal_ensemble_coeff` and the rollout-specific `--policy.n_action_steps=1` override.

Purpose:

Temporal ensembling averages overlapping ACT action chunks across time. It reduces chunk-boundary jitter and policy uncertainty jitter. It is not a motor speed limit and does not replace correct starting pose, camera mapping, or dataset quality.

## Manipulation Agent Reuse

Manipulation Agent must not implement its own LeRobot naming logic.

Rules:

- Manipulation Agent may provide `rollout_dataset_repo_id`, `lerobot_rollout_dataset_repo_id`, or `dataset_repo_id`.
- Manipulation Agent may provide `continuous_rollout`.
- Manipulation Agent may provide `policy_type=pi05`, `policy_type=xvla`, `policy_type=smolvla`, or `policy_type=act`; policy-specific command construction remains inside `LeRobotBridge`.
- Manipulation Agent may provide `rollout_action_clamp` and `rollout_max_relative_target`.
- Manipulation Agent may provide `rollout_temporal_ensemble` and `rollout_temporal_ensemble_coeff`.
- `device_bridges/lerobot_bridge.py` remains the single enforcement point for `eval_` naming, local policy-path normalization, and manual-stop rollout conversion.
- This keeps GUI rollout and autonomous Manipulation Agent rollout consistent.

## Pi0.5 Transfer Rollout Rule

When the workflow reaches Manipulation after a ready 3DP specimen, the default robot-transfer policy is Pi0.5 LeRobot rollout unless the experiment spec explicitly requests `fixed_kinematic`.

Rules:

- Manipulation Agent sends `policy_type=pi05`.
- Test mode may use `policy_path=fake://pi05_policy`.
- Live mode requires a real `policy_path`, `policy_checkpoint_path`, or `policy_repo_id`.
- The LeRobot bridge runs Pi0.5 rollout in conda env `lerobot-pi05-torch211`.
- Pi0.5 rollout command preview uses `lerobot-rollout`, `--policy.type=pi05`, `--device=<device>`, and `--inference.type=rtc` by default.
- On the ROBOTIS OMX-AI follower, Pi0.5 live rollout must pass
  `--robot.disable_torque_on_disconnect=false`. A successful rollout can
  otherwise be converted into a failed GUI session if a Dynamixel reports a
  transient hardware-error bit while LeRobot disables torque during disconnect.
  If a motor is later missing during handshake, inspect/reboot that motor rather
  than silently dropping it from the expected 11-16 follower set.
- The task text must mention the concrete transfer target: `3dp_output_area -> utm_fixture`.

References:

- https://huggingface.co/docs/lerobot/main/en/inference
- https://huggingface.co/docs/lerobot/pi05
