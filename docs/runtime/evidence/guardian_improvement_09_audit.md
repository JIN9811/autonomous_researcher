...
| Test loop cap precedence | Implemented | Recoverable graph-gate pressure cannot extend the deterministic TEST loop beyond the planned 20 cycles. | `tests/unit/test_guardian_agent.py`, `tests/integration/test_controller_run.py` |
| TEST runtime cadence | Implemented | Controller-driven TEST runs use zero inter-stage sleep so 20-cycle safety regressions complete deterministically without weakening Live mode pacing. | `tests/integration/test_controller_run.py` |
| Controller integration timeout | Implemented | The full controller integration test allows 1200 seconds for the 20-cycle geometry, analysis, and BO path under full-suite resource contention. | `tests/integration/test_controller_run.py` |
