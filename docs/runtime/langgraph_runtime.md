...
- BO benchmark/run: `tool.completed` and, for agent-mediated BO, `node.completed` with `node_id=bo` and `module_id=bo`. BO workspace runs create `workspace/bo/*_posterior.png`, `*_posterior.svg`, and `*_posterior.csv` from the shared `bo_visualization.v1` projection so the Runtime IDE can preview the publication figure and audit its exact numeric source. A compact `*_bo_progress.svg` remains only for legacy results without the shared projection. Agent-mediated BO also stores `bo_reasoning_report.json`, `candidate_pool.json`, and `bo_next_candidate.json`, and exposes `bo_result.reasoning`, `candidate_ranking`, `failure_model`, and `next_design_request.v1` in the event payload.
...
- Printer autoejection test: `tool.completed` with `node_id=specimen`. Bambu Lab X2D autoejection is represented as native G-code patch/validation evidence when provider `bambu_gcode_patch` is active. The event must include patched artifact path, validation blockers, source/internal plate path, and bed-clear gate state. Manipulation Agent handoff is reserved for failed-ejection recovery or downstream specimen transfer, not the primary Bambu ejection path.
...
Closed-loop graph runs also materialize runtime evidence for stages that otherwise produced only nested payloads. BO stage results write `runs/<run-id>/runtime/bo/*_bo_agent_result.json` plus `runtime/bo/*_posterior.{png,svg,csv}` from the shared projection contract. LHS cycles render the two-variable initial design; acquisition cycles render posterior mean, 95% confidence interval, observations, next point, and Expected Improvement. Analysis stage results write `runs/<run-id>/runtime/analysis/*_analysis_agent_result.json` and copy CAE input/report/contour files from `artifacts/cae` into the run directory. These files are emitted through `artifact.created` events and stored in `state.run_metadata.runtime_artifacts`, so the Runtime IDE can show BO plots and CAE contour/report outputs for the main 20-cycle closed-loop run, not only for dedicated BO/CAE workspace buttons.
...
- Runtime IDE large-display reference-fit audit was also run at 2560 px width, with the canvas expanding instead of staying fixed-width. Evidence: `artifacts/ui/runtime_ide_reference_fit_2560x1440.png`.
