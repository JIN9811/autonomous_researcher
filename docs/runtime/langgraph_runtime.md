...
Closed-loop graph runs also materialize runtime evidence for stages that otherwise produced only nested payloads. BO stage results write `runs/<run-id>/runtime/bo/*_bo_agent_result.json` plus `runtime/bo/*_posterior.{png,svg,csv}` from the shared posterior/acquisition projection. Analysis stage results write `runs/<run-id>/runtime/analysis/*_analysis_agent_result.json` and copy CAE input/report/contour files from `artifacts/cae` into the run directory. These files are emitted through `artifact.created` events and stored in `state.run_metadata.runtime_artifacts`, so the Runtime IDE can show BO plots and CAE contour/report outputs for the main 5-cycle closed-loop run, not only for dedicated BO/CAE workspace buttons.
