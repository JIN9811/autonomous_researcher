...
- Device-specific live execution checks remain inside printer, LeRobot, Windows equipment and Guardian code. The module approval gate is an additional graph-level operator gate, not a replacement for device bridge safety.
...
Dedicated workspace APIs remain separate from the main orchestration run and emit scoped runtime events and artifacts through `MainController.emit_workspace_result(...)`.
...
- `device_bridge_plane`: hardware/software boundary for PrusaLink, LeRobot/Pi0.5, Windows PyAutoGUI/UTM and camera/UTM evidence tools.
- `memory_evidence_plane`: Knowledge/Graphify/BO evidence plane for experiment evidence, BO observations, and improvement proposals.
...
7. Inspect runtime events, node state, approvals, timeline and scoped artifact previews using a non-actuating test fixture.
...
12. Inspect `/live` to verify measured Analysis cards and objective-bound BO visualization without starting devices.
