...
function liveAgentModuleHostServices() {
  return {
    ...
    latestManipulationReport,
    latestRobotTaskResult,
    latestVisionReport,
    latestVisionSignalPacket,
    latestVisionAgentReport,
    visionSpecimenInterventionFor,
    latestActiveCamArtifact,
    utmVerificationScope,
    updateUtmVerificationSelection,
    selectVerification,
    renderVisionUtmPlacementConfirmation,
    renderVisionSpecimenIntervention,
    renderVisionUtmVerification,
    visionLiveCameraProfile,
    visionLiveFrameEvidence,
    renderVisionRuntimeControls,
    renderVisionLiveFrameEvidence,
    renderVisionCardDetails,
    renderVisionInspectionFeed,
    renderVisionSegmentationPanels,
    renderVisionRuntimeHeaderActions,
    renderVisionActiveCamEjectionCheck,
    renderVisionUtmVerificationTabs,
    renderVisionCameraRuntimeSummary,
    renderVisionCameraHealthBoard,
    renderVisionRuntimeNodeFlow,
    renderVisionHandoffSignal,
    renderVisionEvidenceReviewBoard,
    renderVisionAgenticProgress,
    renderVisionConfusionMatrix,
    renderMiniBarChart,
    renderVisionCardDetails,
    renderGateStatusBars,
    renderVizEmpty,
    visionRuntimeStatus: () => liveUtmRuntimeStatus,
...
async function hydrateLiveSelectedAgentReport(session, manifestGeneration = liveAgentManifestRequestGeneration) {
  ...
  try {
    ...
    const analysis = sections.analysis_report && typeof sections.analysis_report === "object" ? sections.analysis_report : {};
    const reportIdentity = agentId === "bo"
      ? (sections.bo_result && typeof sections.bo_result === "object" ? sections.bo_result : {})
      : analysis.bo_observation && typeof analysis.bo_observation === "object"
        ? analysis.bo_observation
        : sections.bo_observation && typeof sections.bo_observation === "object"
          ? sections.bo_observation
          : analysis.bo_handoff && typeof analysis.bo_handoff === "object"
            ? analysis.bo_handoff
            : sections.bo_handoff && typeof sections.bo_handoff === "object"
              ? sections.bo_handoff
              : {};
...
function latestReportBoResult(report) {
  ...
  const metadata = state && typeof state.run_metadata === "object" && state.run_metadata ? state.run_metadata : {};
  const ownerSections = report && report.sections && typeof report.sections === "object" ? report.sections : {};
  let boResult = ownerSections.bo_result && typeof ownerSections.bo_result === "object"
    ? ownerSections.bo_result
    : null;
  if (!boResult && metadata.bo_agent && typeof metadata.bo_agent === "object") {
    boResult = metadata.bo_agent;
  }
  if (!boResult) {
    const projected = latestReportPayload(report, ["bo_result", "data.bo_result", "sections.bo_result", "role_specific.bo_result"]);
    if (projected && typeof projected === "object" && Object.keys(projected).length) {
      boResult = projected;
    }
  }
  ...
  if (!boResult) {
    boResult = latestReportPayload(report, ["bo_result", "bayesian_optimization", "optimization_result"]);
  }
  return metadataVisualization
    ? { ...(boResult && typeof boResult === "object" ? boResult : {}), visualization: metadataVisualization }
    : boResult;
}

function resolveLiveBoVisualization(report, boResult) {
  const reportRunId = String(report?.state?.run_id || liveCurrentRunId());
  const visualization = preferredLiveBoVisualization(
    boResult && boResult.visualization,
    liveBoVisualization,
    reportRunId,
  );
  if (window.BOVisualization && window.BOVisualization.isValid(visualization)) {
...
function renderSpecimenReportDetails(report) {
  const frontend = liveAgentModuleHost.get("specimen");
  return frontend && typeof frontend.renderReport === "function" ? frontend.renderReport(report) : "";
}

function renderVisionReportDetails(report, status, agentLabel, profile) {
  const frontend = liveAgentModuleHost.get("vision");
  return frontend && typeof frontend.renderReport === "function" ? frontend.renderReport(report, status, agentLabel, profile) : "";
}

function renderManipulationReportDetails(report, status, agentLabel, profile) {
  const frontend = liveAgentModuleHost.get("manipulation");
  return frontend && typeof frontend.renderReport === "function" ? frontend.renderReport(report, status, agentLabel, profile) : "";
}


function renderEquipmentReportDetails(report) {
  const frontend = liveAgentModuleHost.get("equipment");
  return frontend && typeof frontend.renderReport === "function" ? frontend.renderReport(report) : "";
}
...
function renderBoReportDetails(report) {
  const frontend = liveAgentModuleHost.get("bo");
  return frontend && typeof frontend.renderReport === "function" ? frontend.renderReport(report) : "";
}
...
function renderSpecimenPrintControlPanel(report, status, agentLabel, profile) {
  const frontend = liveAgentModuleHost.get("specimen");
  return frontend && typeof frontend.renderDashboard === "function" ? frontend.renderDashboard(report, status, agentLabel, profile) : "";
}
...
function equipmentRuntimeState() {
  return {
    snapshot: liveEquipmentRuntimeSnapshot,
    skillFlowSnapshot: liveEquipmentSkillFlowSnapshot,
    refreshedAt: liveEquipmentRuntimeRefreshedAt,
    error: liveEquipmentRuntimeError,
    actionInFlight: liveEquipmentRuntimeActionInFlight,
    currentRunId: liveCurrentRunId(),
    runArtifacts: liveRunArtifacts,
  };
}
...
if (liveAgentBinderList) {
  ...
  liveAgentBinderList.addEventListener("click", (event) => {
    ...
    setLiveView("report");
    renderLiveRuntime(liveLastSession);
    if (["analysis", "bo"].includes(liveSelectedAgent)) {
      const selectedAgent = liveSelectedAgent;
      const selectedSession = liveLastSession;
      hydrateLiveSelectedAgentReport(selectedSession).then(() => {
        if (liveSelectedAgent !== selectedAgent || liveLastSession !== selectedSession) return;
        invalidateLiveCenterRender("report");
