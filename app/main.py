...
def _lerobot_guided_device_bridge_summary() -> dict[str, object]:
    """Return LeLab-inspired readiness summary without changing the legacy /lerobot GUI."""
    config = _lerobot_bridge().config_status()
    selected_profile_id = str(config.get("selected_profile_id") or config.get("default_profile_id") or "")
    profiles = [item for item in config.get("profiles", []) if isinstance(item, dict)]
    profile = next((item for item in profiles if str(item.get("profile_id") or "") == selected_profile_id), profiles[0] if profiles else {})
    live_gates = dict(profile.get("live_gate_summary") or config.get("live_gate_summary") or {})
    device_memory = dict(config.get("device_memory") or {})
    memory_profiles = device_memory.get("profiles", {}) if isinstance(device_memory.get("profiles"), dict) else {}
    selected_memory = memory_profiles.get(selected_profile_id, {}) if isinstance(memory_profiles, dict) else {}
    devices = selected_memory.get("devices", {}) if isinstance(selected_memory, dict) and isinstance(selected_memory.get("devices"), dict) else {}
    saved_cameras = devices.get("cameras", {}) if isinstance(devices.get("cameras"), dict) else {}
    camera_map = profile.get("camera_map", {}) if isinstance(profile.get("camera_map"), dict) else {}
    expected_camera_keys = list(dict.fromkeys([*camera_map.keys(), "top", "wrist"]))
    ...
    active_statuses = {"RUNNING", "RECORDING", "TELEOP_ACTIVE", "ROLLOUT_ACTIVE", "TRAINING", "POLICY_ACTIVE", "ACTIVE"}
    active_sessions = [item for item in sessions if str(item.get("status") or "").upper() in active_statuses]
    policies_result = _lerobot_bridge().policies_list({"mode": "test"})
    policies = [item for item in policies_result.get("policies", []) if isinstance(item, dict)] if isinstance(policies_result, dict) else []
    workflow_labels = {
        ...
        "manipulation": "Manipulation Agent",
    }

    def session_workflow(session: dict[str, object]) -> str:
        workflow = str(session.get("workflow") or "")
        if "manipulation" in workflow:
            return "manipulation"
        return workflow

    latest_by_workflow: dict[str, dict[str, object]] = {}
    active_by_workflow: dict[str, list[dict[str, object]]] = {key: [] for key in workflow_labels}
    for session in sessions:
        workflow = session_workflow(session)
        if workflow:
            latest_by_workflow[workflow] = session
        if str(session.get("status") or "").upper() in active_statuses:
            active_by_workflow.setdefault(workflow, []).append(session)

    def session_message(session: dict[str, object]) -> str:
        return str(
            session.get("runtime_message")
            or session.get("runtime_phase")
    ...
    for workflow, label in workflow_labels.items():
        latest = latest_by_workflow.get(workflow, {})
        ...
        workflow_diagnostics[workflow] = {
            ...
            "latest_session_id": str(latest.get("session_id") or "") if latest else "",
            "latest_status": str(latest.get("status") or "") if latest else "idle",
            "latest_message": session_message(latest) if latest else "No session yet.",
            "log_path": str(latest.get("log_path") or "") if latest else "",
        }
    job_cards = []
    for session in list(reversed(sessions[-12:])):
        workflow = session_workflow(session)
        runtime = session.get("runtime") if isinstance(session.get("runtime"), dict) else {}
        training = session.get("training") if isinstance(session.get("training"), dict) else {}
        progress = training.get("progress_percent") if isinstance(training, dict) else None
        job_cards.append(
            {
                "session_id": str(session.get("session_id") or ""),
                "workflow": workflow,
                "label": workflow_labels.get(workflow, workflow or "session"),
                "status": str(session.get("status") or "UNKNOWN"),
                "mode": str(session.get("mode") or ""),
                "profile_id": str(session.get("profile_id") or ""),
                "title": f"{workflow_labels.get(workflow, workflow or 'Session')} · {session.get('status') or 'UNKNOWN'}",
                "detail": session_message(session),
                "progress_percent": progress if isinstance(progress, (int, float)) else None,
                "dataset_path": str(session.get("dataset_path") or ""),
                "checkpoint_path": str(session.get("checkpoint_path") or ""),
                "log_path": str(session.get("log_path") or ""),
                "runtime_phase": str(runtime.get("phase") or session.get("runtime_phase") or "") if isinstance(runtime, dict) else str(session.get("runtime_phase") or ""),
                "created_at": str(session.get("created_at") or ""),
                "tone": "busy" if str(session.get("status") or "").upper() in active_statuses else "ok" if str(session.get("status") or "").upper() == "COMPLETED" else "warning" if str(session.get("status") or "").upper() == "FAILED" else "idle",
            }
        )

    def device_detail(role: str, fallback: str = "") -> str:
        value = devices.get(role, {}) if isinstance(devices, dict) else {}
        if isinstance(value, dict):
            port = value.get("port") or value.get("identity_path") or value.get("device_path") or ""
            if port:
                return str(port)
        return fallback

    def readiness_item(item_id: str, label: str, status: str, detail: str, *, tone: str = "idle") -> dict[str, object]:
        return {"id": item_id, "label": label, "status": status, "detail": detail, "tone": tone}

    follower_detail = device_detail("follower") or device_detail("robot") or str(profile.get("robot_port") or "not saved")
    leader_detail = device_detail("leader") or device_detail("teleop") or str(profile.get("teleop_port") or "not saved")
    camera_ready_count = len(saved_cameras)
    camera_expected_count = len(expected_camera_keys)
    active_detail = f"{len(active_sessions)} active / {len(sessions)} recent sessions"

    readiness = [
        readiness_item(
            "profile",
            "Robot Profile",
            str(profile.get("display_name") or selected_profile_id or "not configured"),
            f"family={profile.get('robot_family', 'unknown')} · default={config.get('default_profile_id', '')}",
            tone="ok" if profile else "warning",
        ),
        readiness_item(
            "follower",
            "Follower Arm",
            "saved" if follower_detail != "not saved" else "needs setup",
            follower_detail,
            tone="ok" if follower_detail != "not saved" else "warning",
        ),
        readiness_item(
            "leader",
            "Leader Arm",
            "saved" if leader_detail != "not saved" else "needs setup",
            leader_detail,
            tone="ok" if leader_detail != "not saved" else "warning",
        ),
        readiness_item(
            "cameras",
            "Cameras",
            f"{camera_ready_count}/{camera_expected_count} saved",
            ", ".join(expected_camera_keys) or "top, wrist",
            tone="ok" if camera_expected_count and camera_ready_count >= camera_expected_count else "warning",
        ),
        readiness_item(
            "policy",
            "Policy",
            f"{len(policies)} choices",
            str(config.get("paths", {}).get("policy_root", "outputs/train")) if isinstance(config.get("paths"), dict) else "outputs/train",
            tone="ok" if policies else "idle",
        ),
        readiness_item(
            "safety",
            "Safety Gate",
            "live enabled" if live_gates.get("live_enabled") else "test / gated",
            f"teleop={bool(live_gates.get('allow_teleoperation'))} · record={bool(live_gates.get('allow_recording'))} · rollout={bool(live_gates.get('allow_policy_rollout'))}",
            tone="ok" if live_gates.get("live_enabled") else "warning",
        ),
        readiness_item(
            "active_workflow",
            "Active Workflow",
            "busy" if active_sessions else "idle",
            active_detail,
            tone="busy" if active_sessions else "idle",
        ),
    ]
    workflow_actions = [
        {"id": "setup_robot", "label": "Setup Robot", "target": "device_setup", "enabled": True},
        {"id": "teleoperate", "label": "Teleoperate", "target": "teleoperation", "enabled": True},
        {"id": "record_dataset", "label": "Record Dataset", "target": "recording", "enabled": True},
        {"id": "visualize_dataset", "label": "Visualize Dataset", "target": "visualization", "enabled": True},
        {"id": "train_policy", "label": "Train Policy", "target": "training", "enabled": True},
        {"id": "run_rollout", "label": "Run Rollout", "target": "rollout", "enabled": True},
        {"id": "manipulation_agent", "label": "Manipulation Agent", "target": "manipulation", "enabled": True},
    ]
    return {
        "ok": True,
        "bridge": "lerobot_guided_device_bridge",
        "source_bridge": "lerobot",
        ...
        "live_gate_summary": live_gates,
        "legacy_gui_path": "/lerobot",
        "api_prefix": "/api/lerobot",
    }
...
@app.get("/device-bridge/lerobot", response_class=HTMLResponse)
async def lerobot_guided_device_bridge_gui(request: Request) -> HTMLResponse:
    """Serve LeLab-inspired ROBOTIS guided device bridge without modifying /lerobot."""
    return templates.TemplateResponse(
        request=request,
        name="lerobot_device_bridge.html",
        context={"title": "ROBOTIS Guided Device Bridge"},
    )
...
@app.get("/api/device-bridge/lerobot/summary")
async def get_lerobot_guided_device_bridge_summary() -> dict[str, object]:
    """Return the LeLab-inspired ROBOTIS LeRobot device bridge readiness view."""
    return _lerobot_guided_device_bridge_summary()
