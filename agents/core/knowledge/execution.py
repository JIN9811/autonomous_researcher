"""Knowledge's archived owner body as one guarded composite operation."""
...
def default_knowledge_execution_graph():
    return installed_execution_graph("knowledge")


def knowledge_execution_catalog(agent):
    async def task(state, ctx, scope, config):
        if scope.get("task_started"):
            raise ExecutionGraphError("Knowledge task cannot repeat within an invocation")
    ...
    async def deliver(state, ctx, scope, config):
        ...
        if not isinstance(result, AgentResult):
            raise ExecutionGraphError("Knowledge delivery requires an owner AgentResult")
    ...
    return ExecutionCatalog(
        ...
        (
            ExecutionOperation(
                "knowledge.task",
                task,
                label="Composite scoped curation, memory and evidence task",
        ...
        implementation_structure=knowledge_implementation_structure,
    )


async def execute_knowledge_graph(agent, state, ctx, *, graph=None, emit=None, invocation=None):
    compiled = compile_execution_graph(
        deepcopy(dict(graph)) if graph is not None else default_knowledge_execution_graph(),
        knowledge_execution_catalog(agent),
    )
