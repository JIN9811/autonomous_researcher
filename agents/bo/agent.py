...
class BOAgent(BaseAgent):
    ...
    def execution_catalog(self):
        from agents.bo.execution import bo_execution_catalog

        return bo_execution_catalog(self)
    ...
    async def _execute(
        ...
        execution = await execute_bo_graph(
            ...
            ctx,
            settings=settings,
            graph=execution_graph_from_context(ctx, "bo", default_bo_execution_graph),
            emit=execution_event_emitter(ctx),
        )
        if not isinstance(execution.result, AgentResult):
            raise RuntimeError("BO execution graph completed without AgentResult")
    ...
    async def run_with_settings(
        ...
        """Run the guarded owner graph with explicit GUI/API settings."""
        return await self._execute(state, ctx, deepcopy(settings or {}))

    @archive_agent_run
    async def run(self, state: OrchestratorState, ctx: AgentContext) -> AgentResult:
        """Run the guarded owner graph with this run's validated settings."""
        current = state.run_metadata.get("bo_settings")
        ...
        return await self._execute(state, ctx, settings)
