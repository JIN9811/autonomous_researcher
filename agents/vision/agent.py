...
from agents.vision.decision import (
...
class VisionAgent(BaseAgent):
    ...
    def execution_catalog(self):
        ...
        return vision_execution_catalog(self)
    ...
    async def run(self, state: OrchestratorState, ctx: AgentContext) -> AgentResult:
        ...
        from agents.vision.execution import default_vision_execution_graph, execute_vision_graph

        execution = await execute_vision_graph(
            self, state, ctx,
            graph=execution_graph_from_context(ctx, "vision", default_vision_execution_graph),
            emit=execution_event_emitter(ctx),
        )
        if not isinstance(execution.result, AgentResult):
            raise RuntimeError("Vision execution graph completed without AgentResult")
        return execution.result

    def _prepare_observation(self, state: OrchestratorState, ctx: AgentContext) -> dict[str, Any] | AgentResult:
        """Admit the existing clearance, stop, preflight or observation route in order."""
        ...
        return {"route": "observe", "frame_id": frame_id, "specimen": specimen, "spec": spec}
    ...
    async def _observe_and_review(self, state: OrchestratorState, ctx: AgentContext, prepared: dict[str, Any]) -> AgentResult:
        ...
        frame_id, specimen, spec = prepared["frame_id"], prepared["specimen"], prepared["spec"]
