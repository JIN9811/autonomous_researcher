...
class ManipulationAgent(BaseAgent):
    ...
    def execution_catalog(self):
        from agents.manipulation.execution import manipulation_execution_catalog

        return manipulation_execution_catalog(self)
    ...
    async def run(self, state: OrchestratorState, ctx: AgentContext) -> AgentResult:
        ...
        execution = await execute_manipulation_graph(
            self, state, ctx,
            graph=execution_graph_from_context(ctx, "manipulation", default_manipulation_execution_graph),
            emit=execution_event_emitter(ctx),
        )
        if not isinstance(execution.result, AgentResult):
            raise RuntimeError("Manipulation execution graph completed without AgentResult")
