...
class SpecimenMakingAgent(BaseAgent):
    ...
    def execution_catalog(self):
        from agents.specimen.execution import specimen_execution_catalog

        return specimen_execution_catalog(self)

    @archive_agent_run
    async def run(self, state: OrchestratorState, ctx: AgentContext) -> AgentResult:
        from agents.specimen.execution import default_specimen_execution_graph, execute_specimen_graph

        execution = await execute_specimen_graph(
            self, state, ctx,
            graph=execution_graph_from_context(ctx, "specimen", default_specimen_execution_graph),
            emit=execution_event_emitter(ctx),
        )
        if not isinstance(execution.result, AgentResult):
            raise RuntimeError("Specimen execution graph completed without AgentResult")
    ...
    def _prepare_fabrication(self, state: OrchestratorState, ctx: AgentContext) -> dict[str, Any] | AgentResult:
        ...
        return {
            "spec": spec, "candidate": candidate, "specimen_id": specimen_id,
            "geometry_result": geometry_result, "mesh_result": mesh_result,
            "manufacturability_result": manufacturability_result, "handoff_result": handoff_result,
            "printer_payload": printer_payload, "evaluation_payload": evaluation_payload,
            "printer_runtime_mode": printer_runtime_mode, "live_gui_test_spec": live_gui_test_spec,
            "printer_test_path": printer_test_path, "top_cap_enabled": top_cap_enabled,
            "bottom_cap_enabled": bottom_cap_enabled, "geometry_payload": geometry_payload,
        }

    async def _decide_fabrication(self, state, ctx, prepared):
        """Keep bounded reasoning and its single execution callback in one operation."""
        spec = prepared["spec"]
        candidate = prepared["candidate"]
        specimen_id = prepared["specimen_id"]
        printer_payload = prepared["printer_payload"]
        evaluation_payload = prepared["evaluation_payload"]
        printer_runtime_mode = prepared["printer_runtime_mode"]
        live_gui_test_spec = prepared["live_gui_test_spec"]
        printer_test_path = prepared["printer_test_path"]
        ...
        specimen_decision, experiment_response = await decide_specimen(
            ...
            fabrication_evidence(spec, prepared["geometry_result"], prepared["mesh_result"], prepared["manufacturability_result"],
                                 evaluation_payload["execution"], printer_payload), execute_fabrication,
        )
        return {"specimen_decision": specimen_decision, "experiment_response": experiment_response}

    def _review_fabrication(self, prepared, decision):
        specimen_decision = decision["specimen_decision"]
        return AgentResult(
            ...
            data={"specimen_decision": specimen_decision,
                  ...
                  "specimen_result": {"ok": False, "status": "blocked", "candidate_id": prepared["candidate"],
                                      "specimen_id": prepared["specimen_id"], "specimen_decision": specimen_decision}},
        )

    def _finalize_fabrication(self, state, prepared, decision):
        """Project the completed callback response without replaying fabrication."""
        spec = prepared["spec"]
        candidate = prepared["candidate"]
        specimen_id = prepared["specimen_id"]
        geometry_result = prepared["geometry_result"]
        mesh_result = prepared["mesh_result"]
        manufacturability_result = prepared["manufacturability_result"]
        handoff_result = prepared["handoff_result"]
        printer_payload = prepared["printer_payload"]
        printer_runtime_mode = prepared["printer_runtime_mode"]
        live_gui_test_spec = prepared["live_gui_test_spec"]
        printer_test_path = prepared["printer_test_path"]
        top_cap_enabled = prepared["top_cap_enabled"]
        bottom_cap_enabled = prepared["bottom_cap_enabled"]
        geometry_payload = prepared["geometry_payload"]
        specimen_decision = decision["specimen_decision"]
        experiment_response = decision["experiment_response"]
