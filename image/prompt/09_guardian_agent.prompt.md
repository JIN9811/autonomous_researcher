...
핵심 표현:
- Guardian Agent를 physical execution safety/policy gate로 표현한다. design constraints, printer state, PrusaLink metadata start path, LeRobot live gates, Windows bridge allowlist, continue/stop/error transition을 강조한다.

## Role
- 전체 loop의 안전성, 실행 권한, 다음 transition을 최종 판정한다.

## Inputs
- current state and tool call
- experiment_spec
- specimen_result
- device status
- safety validation / bounds
- failure memory

## Core Actions
- Validate constraints
- Check live/test gates
- Inspect printer start path
- Check robot ports/cameras
- Check Windows allowlist
...
- PrusaLink checks
- LeRobot registry
- Windows bridge registry

## Safety/Contract
- Default deny unknown hardware
- Reject unsafe device state
- Require short-name PrusaLink start
- No unsupported SetReady assumption
