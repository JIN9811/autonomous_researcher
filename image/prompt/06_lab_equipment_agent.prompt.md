...
핵심 표현:
- Lab Equipment Agent를 Windows PyAutoGUI bridge와 allowlisted macro runner 담당자로 표현한다. token verified discovery, scanned candidate save, quick select/delete, Windows-owned program1 execution, Linux GUI/API control boundary를 강조한다.

## Role
- Windows PC의 GUI 장비 프로그램을 안전한 HTTP bridge와 allowlisted macro로 실행한다.

## Inputs
- selected Windows bridge
- token host/port memory
- program_id such as program1
- structured macro args
- health state

## Core Actions
- Scan token-matching hosts
- Save quick-select candidate
- Health check and list programs
- Run allowlisted macro
...
- equipment_result
- program1 status
- Windows step_trace/log
- protocol_note

## Tools/Interfaces
- pyautogui.health/discover
- pyautogui.list/run
...
## Safety/Contract
- Windows owns live macros
- Token-verified internal network
- No shell/Python/PowerShell/eval
- Fail safe on lost health
