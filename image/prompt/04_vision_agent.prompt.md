...
핵심 표현:
- Vision Agent를 typed observation provider로 표현한다. top/wrist cameras, LeRobot dataset frames, simulator sources, specimen/equipment observations, uncertainty metadata, downstream manipulation/guardian feedback를 강조한다.

## Role
- 카메라, dataset, simulator에서 검증 가능한 observation을 만들고 다음 stage에 전달한다.

## Inputs
- current experiment state
- top/wrist camera config
- LeRobot dataset samples
- specimen/equipment request
- capture schema
...
## Safety/Contract
- Read-only hardware behavior
- No gate bypass
- Fail closed on ambiguous camera
- No contact inference without evidence
